...
local function v_u_119()
	...
	for _, v109 in ipairs(v104) do
		...
		if (v110 ~= "Staff" or v92) and ((v110 ~= "Leaderboards" or v91) and #v112 > 0) then
			...
			for _, v117 in ipairs(v112) do
				if (v117.config.unlockType ~= "staff" or table.find(v90, v117.id)) and (v117.config.unlockType ~= "leaderboard" or table.find(v90, v117.id)) then
					local v118 = v_u_89(v117.id, v117.config, v111)
					if v118 then
						v_u_73(v118, v117.id)
						v118.Parent = v_u_20
					end
...
local function v121()
	-- upvalues: (copy) v_u_6, (ref) v_u_23, (copy) v_u_15, (copy) v_u_119
...
v14.OnClientEvent:Connect(v121)
v11.OnClientEvent:Connect(function(p122)
...
v18.Visible = false
(function()
	-- upvalues: (copy) v_u_19, (copy) v_u_120
	local v123 = v_u_19:FindFirstChild("CloseButtonFrame")
	if v123 then
		local v124 = v123:FindFirstChild("CloseButton")
		if v124 then
			local v125 = v124:FindFirstChild("LocalScript")
			if v125 then
				v125:Destroy()
			end
			v124.MouseButton1Click:Connect(v_u_120)
		end
	end
end)()